...
L("""async function createOrders(custIds) {
    ...
    try {
        ...
        for (const o of orders) {
            ...
            const detId = uuid();
            await client.query("INSERT INTO order_detail(id,order_id,product_name,product_type,cabinet_count,board_count,has_boards,material,color,length,width,height,unit_price,quantity,amount,created_at) VALUES($1,$2,'标准橱柜','定制品',2,8,true,'304不锈钢','银色',800,600,2000,1,1,$3,NOW())", [detId, id, o.amt]);
            const trkId = uuid();
            await client.query("INSERT INTO order_tracking(id,order_id,current_stage,stage_name,stage_status,operator_name,started_at,created_at) VALUES($1,$2,'design','设计阶段','in_progress','王五',NOW(),NOW())", [trkId, id]);
            const bomId = uuid();
            await client.query("INSERT INTO order_bom(id,order_id,material_code,material_name,material_type,specification,unit,quantity,unit_price,total_price,remark,created_at) VALUES($1,$2,'MAT-SUS15','不锈钢板','原材料','1.5mm','张',10,380,3800,NULL,NOW())", [bomId, id]);
...
L("""async function createStock(matIds, whIds) {
    ...
    try {
        ...
        if (zeroMat) {
            var inId = uuid();
            await client.query("INSERT INTO stock_in(id,stock_in_no,warehouse_id,supplier_id,operator_name,total_amount,remark,created_at) VALUES($1,$2,$3,NULL,'陈四',1200,'边界-零库存进10出10',NOW())", [inId, "IN" + ts().slice(-6), whIds[0].id]);
            var invId = uuid();
            ...
            var outId = uuid();
            await client.query("INSERT INTO stock_out(id,stock_out_no,warehouse_id,operator_name,total_amount,remark,created_at) VALUES($1,$2,$3,'周九',1200,'边界-零库存出库',NOW())", [outId, "OUT" + ts().slice(-6), whIds[0].id]);
...
L("""async function createProduction(orders) {
    ...
    try {
        ...
        if (orders[0]) { var doorId2 = uuid(); await client.query("INSERT INTO door_panel_production(id,order_id,door_no,door_type,length,width,thickness,weight,quantity,material,color,finish,has_handle,has_hinge,hinge_count,coating_type,coating_color,baking_temp,baking_time,status,cutting_complete,quality_start,quality_complete,quality_result,quality_remark,current_location,created_at) VALUES($1,$2,$3,'标准门板',800,600,18,25.5,2,'304不锈钢','银色','拉丝',true,'液压',6,'粉末喷涂','银色',180,30,'in_production',CURRENT_TIMESTAMP-INTERVAL'5 days',CURRENT_TIMESTAMP-INTERVAL'1 day',CURRENT_TIMESTAMP,'rejected','涂层气泡超标，已返工2次','待返工区',NOW())", [doorId2, orders[0].id, "D" + ts().slice(-6) + "BAD"]); console.log("OK 边界: 门板质量不合格 [rejected]"); }
        if (orders[1]) { var ctId2 = uuid(); await client.query("INSERT INTO countertop_production(id,order_id,production_no,countertop_type,material,length,width,thickness,color,edge_style,has_sink_hole,has_faucet_hole,quantity,status,cutting_complete,quality_start,quality_complete,quality_result,current_location,created_at) VALUES($1,$2,$3,'石英石台面','石英石',2000,600,15,'白色','磨边',true,true,1,'in_production',CURRENT_TIMESTAMP-INTERVAL'4 days',CURRENT_TIMESTAMP-INTERVAL'1 day',CURRENT_TIMESTAMP,'scrapped','废料区',NOW())", [ctId2, orders[1].id, "CT" + ts().slice(-6) + "SCRP"]); console.log("OK 边界: 台面报废 [scrapped]"); }
        await client.query("COMMIT");
    } catch (err) { await client.query("ROLLBACK"); throw err; }
    finally { client.release(); }
}""")
...
L("""async function createPackages(orders) {
    ...
    try {
        ...
        for (var pai = 0; pai < Math.min(3, orders.length); pai++) {
            ...
            for (var paj = 0; paj < Math.min(3, totalPkgs); paj++) { var itemRemark = damagedPkgs > 0 && paj < damagedPkgs ? "轻微破损" : "正常"; await client.query("INSERT INTO package_item(id,package_id,product_name,product_type,quantity,weight,remark,created_at) VALUES($1,$2,$3,'橱柜',1,$4,$5,NOW())", [uuid(), pkgId, pick(["门板","台面","铰链","导轨","拉手"]), rf(5,30), itemRemark]); }
...
L("""async function createAttendance(empIds) {
    ...
    try {
        ...
        for (var ai = 0; ai < empIds.length; ai++) {
            var emp = empIds[ai];
            for (var ad = 0; ad < 20; ad++) { var d = new Date(); d.setDate(d.getDate() - ad); var dateStr = d.toISOString().split("T")[0]; await client.query("INSERT INTO attendance_record(id,employee_id,record_date,status,check_in_time,check_out_time,working_hours,remark,created_at) VALUES($1,$2,$3,'normal',$4,$5,8,'正常',NOW())", [uuid(), emp.id, dateStr, "08:0" + String(ad % 10), "18:0" + String(ad % 10)]); }
            for (var aa = 21; aa <= 23; aa++) { var da = new Date(); da.setDate(da.getDate() - aa); await client.query("INSERT INTO attendance_record(id,employee_id,record_date,status,working_hours,remark,created_at) VALUES($1,$2,$3,'absent',0,'边界-旷工',NOW())", [uuid(), emp.id, da.toISOString().split("T")[0]]); }
            var otDate = new Date(); otDate.setDate(otDate.getDate() - 24);
            await client.query("INSERT INTO attendance_record(id,employee_id,record_date,status,check_in_time,check_out_time,working_hours,remark,created_at) VALUES($1,$2,$3,'normal','08:00','22:30',14,'边界-日加班6小时',NOW())", [uuid(), emp.id, otDate.toISOString().split("T")[0]]);
            for (var al = 25; al <= 29; al++) { var dl = new Date(); dl.setDate(dl.getDate() - al); await client.query("INSERT INTO attendance_record(id,employee_id,record_date,status,check_in_time,check_out_time,working_hours,remark,created_at) VALUES($1,$2,$3,'late','09:30','18:00',7,'边界-迟到',NOW())", [uuid(), emp.id, dl.toISOString().split("T")[0]]); }
        }
...
L("""async function createFinance(orders) {
    ...
    try {
        ...
        if (overOrder) { await client.query("INSERT INTO receivable(id,dealer_id,order_id,bill_no,bill_date,due_date,amount,paid_amount,status,remark,created_at) VALUES($1,$2,$3,'BILL'+NOW()::text,DATE'2026-03-01',DATE'2026-03-15',$4,0,'overdue','边界-逾期20天',NOW())", [overId, "00000000-0000-0000-0000-000000000000", overOrder.id, rf(10000, 30000)]); console.log("OK 边界财务: 逾期应收账款 [overdue, 逾期20天]"); }
        var badId = uuid(); var badOrder = orders.find(o => o.total > 0);
        if (badOrder) { await client.query("INSERT INTO receivable(id,dealer_id,order_id,bill_no,bill_date,due_date,amount,paid_amount,status,remark,created_at) VALUES($1,$2,$3,'BILL'+NOW()::text,DATE'2026-01-01',DATE'2026-01-15',$4,0,'bad_debt','边界-坏账(客户破产)',NOW())", [badId, "00000000-0000-0000-0000-000000000000", badOrder.id, rf(20000, 50000)]); console.log("OK 边界财务: 坏账 [bad_debt]"); }
