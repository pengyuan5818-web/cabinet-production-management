...
# New verifyReports - each query wrapped in try-catch so we can isolate the error
new_fn = '''async function verifyReports() {
    ...
    await safeQuery("考勤统计", "SELECT e.employee_name, " +
    ...
    await safeQuery("库存异常", "SELECT m.material_name, w.warehouse_name, " +
    ...
    await safeQuery("应收异常", "SELECT order_id, CAST(amount AS text), " +
        "CAST(paid_amount AS text), status, remark " +
        "FROM receivable WHERE status IN ('overdue', 'bad_debt')");
    await safeQuery("物流异常", "SELECT logistics_no, logistics_company, status, remark " +
        "FROM logistics_record WHERE status IN ('rejected', 'returned')");
    await safeQuery("安装异常", "SELECT task_no, status, accept_status, visit_status " +
        "FROM installation_task WHERE status IN ('rework', 'failed') OR visit_status = 'visited'");
    await safeQuery("包装记录", "SELECT pr.package_no, CAST(pr.total_packages AS int), " +
        "(SELECT COUNT(*) FROM package_item pi WHERE pi.package_id = pr.id) as item_count " +
        "FROM package_record pr");
    await safeQuery("付款记录", "SELECT payment_no, CAST(amount AS text), payment_method, status, remark " +
