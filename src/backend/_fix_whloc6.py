...
old_block = """    const { location_name } = req.body;
...
new_block = """    const { location_code, location_name } = req.body;
...
if old_block in content:
    ...
    print('Backend fixed OK')
else:
    print('ERROR: block not found')
    idx = content.find('const { location_name } = req.body;')
    if idx >= 0:
